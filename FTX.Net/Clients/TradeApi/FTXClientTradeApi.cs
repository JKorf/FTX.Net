...
#pragma warning disable 1066
        ...
        async Task<WebCallResult<OrderId>> IFuturesClient.PlaceOrderAsync(string symbol, CryptoExchange.Net.ComonObjects.OrderSide side, CryptoExchange.Net.ComonObjects.OrderType type, decimal quantity, decimal? price, int? leverage, string? accountId)
        {
            ...
            var order = await Trading.PlaceOrderAsync(
                symbol,
                side == CryptoExchange.Net.ComonObjects.OrderSide.Buy ? Enums.OrderSide.Buy : Enums.OrderSide.Sell,
        ...
        async Task<WebCallResult<Order>> IBaseRestClient.GetOrderAsync(string orderId, string? symbol = null)
        {
            ...
            return order.As(new Order
            {
                ...
                Status = (order.Data.Status == Enums.OrderStatus.Open || order.Data.Status == Enums.OrderStatus.New) ? CryptoExchange.Net.ComonObjects.OrderStatus.Active: order.Data.QuantityRemaining > 0 ? CryptoExchange.Net.ComonObjects.OrderStatus.Canceled: CryptoExchange.Net.ComonObjects.OrderStatus.Filled,
                Type = order.Data.Type == Enums.OrderType.Market ? CryptoExchange.Net.ComonObjects.OrderType.Market: CryptoExchange.Net.ComonObjects.OrderType.Limit
            });
        }
        ...
        async Task<WebCallResult<IEnumerable<Order>>> IBaseRestClient.GetOpenOrdersAsync(string? symbol = null)
        {
            ...
            return orders.As(orders.Data.Select(t => new Order
            {
                ...
                Status = (t.Status == Enums.OrderStatus.Open || t.Status == Enums.OrderStatus.New) ? CryptoExchange.Net.ComonObjects.OrderStatus.Active : t.QuantityRemaining > 0 ? CryptoExchange.Net.ComonObjects.OrderStatus.Canceled : CryptoExchange.Net.ComonObjects.OrderStatus.Filled,
                Type = t.Type == Enums.OrderType.Market ? CryptoExchange.Net.ComonObjects.OrderType.Market : CryptoExchange.Net.ComonObjects.OrderType.Limit
            }));
        }
